...
def evaluate_and_plot(q_table_path='Q_table.pickle',
    ...
    for ep in range(1, num_episodes+1):
        ...
        ep_reward = 0
        while not done:
            st = compute_partial_state_hash(env.current_state['player_position'],
                                            env.wall_positions,
                                            env.grid_size)
        ...
        if ep % 500 == 0 or ep == 1:
            ...
            print(f"Episode {ep:4d}  Reward {ep_reward:6.2f}  "
                  f"{ma_window}-ep moving avg {ma:6.2f}")
    env.close()
    ...
    ax.set_title('Agent Performance Over Episodes', fontsize=title_fs)
    ax.legend(fontsize=label_fs)
    ax.grid(**grid_kw)
    ax.tick_params(axis='both', labelsize=tick_fs)
    plt.tight_layout()
    fig.savefig('reward_curve.png')  # saves high‑res PNG
    plt.show()
    ...
    ax.set_title('Distribution of Episode Rewards', fontsize=title_fs)
    ax.grid(**grid_kw)
    ...
    ax.set_title('Boxplot of Episode Rewards', fontsize=title_fs)
